...
% Written by Marisa Lim (marisa.lim@u.nus.edu)
% .txt files from sensor required, delimiter: space
...
fclose(fileID2);
x2(:,1) = data2{:,1}+42113+1;% column 1: cycle vector
x2(:,2) = data2{:,2};% column 2: pressure data in kPa

filename3 = '_nrp finger 14959 cycles_.txt';
fileID3 = fopen(filename3);
data3 = textscan(fileID3, '%f %f');
fclose(fileID3);
x3(:,1) = data3{:,1}+42113+11137+2; % column 1: cycle vector
x3(:,2) = data3{:,2};% column 2: pressure data in kPa

filename4 = '_nrp finger 3103 cycles_.txt';
fileID4 = fopen(filename4);
data4 = textscan(fileID4, '%f %f');
fclose(fileID4);
x4(:,1) = data4{:,1}+42113+11137+14959+3; % column 1: cycle vector
x4(:,2) = data4{:,2};% column 2: pressure data in kPa

x = cat(1, x1, x2, x3, x4); %concatenate individual arrays
