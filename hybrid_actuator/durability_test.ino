...
Adafruit_ADS1015 ads = Adafruit_ADS1015(0x49);     /* Use thi for the 12-bit version */
Adafruit_ADS1015 ads2 = Adafruit_ADS1015(0x48);     /* Use thi for the 12-bit version */
...
int PRESSURE[11];
int value[10];
int VIN_SENSE; //11 ellements array for holding PRESSURE readings
...
void read_while_rest(int setTime) {
  startT = millis();
  while (millis() - startT < setTime) {
    ADC_READING();
  }
}
...
void cyclic_valve() {
  for (int i = 0; i < 5000; i++) {
    TNTPWM.set_channel_value(1, 4095);//on
    read_while_rest(period / 2);
    TNTPWM.set_channel_value(1, 0);//off
    read_while_rest(period / 2);
    count = count + 1;
  }
}
...
void ADC_READING() {
  ...
  if ((PRESSURE[2] <= 263) && (PRESSURE[2] > 0)) {
    val = 0;
  }
